...
for attempt in \$(seq 1 180); do
    if juju status -m "$MODEL" --format=json | cat | python3 -c 'import json, sys; d=json.load(sys.stdin); apps=d.get("applications", {}); provider=apps.get(sys.argv[1], {}); app=apps.get(sys.argv[2], {}); expected=int(sys.argv[3]); provider_units=provider.get("units", {}); app_units=app.get("units", {}); good_provider=provider.get("status") == "active" and len(provider_units) >= expected and all(u.get("agent-status") == "idle" and u.get("workload-status") == "active" for u in provider_units.values()); good_app=app.get("status") == "active" and bool(app_units) and all(u.get("agent-status") == "idle" and u.get("workload-status") == "active" for u in app_units.values()); sys.exit(0 if good_provider and good_app else 1)' "$PROVIDER" "$APP" "$orig_units"; then
        exit 0
    fi
    sleep 5
done
