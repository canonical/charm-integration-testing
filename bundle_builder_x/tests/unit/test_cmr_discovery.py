...
"""Tests that the Z3 solver discovers cross-model integrations via PotentialCMRs."""
...
class TestSolverDiscoversCMR:
    def test_solver_activates_potential_cmr_to_satisfy_requires(self) -> None:
        ...
        PROVIDES endpoint, the solver should activate the PotentialCMR."""
        # GIVEN two models
        ...
        # AND the PotentialCMR is activated
        assert len(domain.potential_cmrs) == 1
        pcmr = domain.potential_cmrs[0]
        assert model.evaluate(pcmr.exists, model_completion=True)
    ...
    def test_solver_does_not_activate_unnecessary_cmr(self) -> None:
        """When both endpoints can be satisfied locally, no PotentialCMR is activated."""
        domain = initialize_global_domain(
            {
                "m1": ModelInit(
                    applications={
                        "pg": ApplicationConstraint(charm="postgresql"),
                        "proxy": ApplicationConstraint(charm="pgbouncer"),
                    },
                    platform="kubernetes",
                    arch="amd64",
                    juju_version=_JUJU,
                ...
                "m2": ModelInit(
                    applications={"other": ApplicationConstraint(charm="other-charm")},
                    platform="kubernetes",
                    arch="amd64",
                    juju_version=_JUJU,
                ),
        ...
        add_charm_to_domain(other, domain, "m2")
        ...
        # PotentialCMRs exist (cross-model matches) but should not all be activated
