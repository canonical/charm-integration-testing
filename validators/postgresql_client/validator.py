...
class PostgreSQLClientPersistenceValidator(_PostgreSQLConnectionMixin, BasePersistenceValidator):
    ...
    it - see ``cleanup()``. Every row written to the table also carries a random, unguessable
    ``token`` generated by ``prepare()`` and carried forward as ``PersistenceState.token``, so
    ``checkpoint()`` can detect data loss (or a table silently recreated from scratch) by counting
    only rows tagged with that token, rather than trusting a plain ``count(*)`` that a
    coincidentally-sized but unrelated table could satisfy. The token must be random rather than
    derived from ``identifier``/``ref``: those are reproducible, so a backend that lost the canary
    data and recreated it from scratch (resetting the ``SERIAL`` sequence, for example) would
    reproduce the same value and pass falsely. ``ref`` tracks how many tagged rows are expected so
    far.
        ...
        try:
            with conn.cursor() as cur:
                ...
                if schema is None:
                    ...
                    # The token itself is random per prepare() run, so a table dropped and recreated
                    # from scratch (resetting the SERIAL sequence) can't reproduce it either.
                    cur.execute(
                ...
                # Only write the next canary row when this checkpoint passed: ValidatorRunner
