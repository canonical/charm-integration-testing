...
class ProblemSpace(BaseModel):
    ...
    constraint_names: dict[str, tuple[str, str, str]] = Field(default_factory=dict)  # name -> (app, endpoint, interface)
...
class BundleBuilder:
    ...
    def build(self, base: Bundle) -> Bundle:
        ...
        while True:
            ...
            self.logger.info(f"Created problem space with {len(problem_space.app_vars)} app vars, {len(problem_space.integration_vars)} integration vars")
            ...
            self.logger.info(f"Added base bundle constraints")
            ...
            while True:
                ...
                if result == z3.sat:
                    ...
                    self.logger.info(f"Z3 found candidate bundle with {len(candidate_bundle.applications)} apps: {list(candidate_bundle.applications.keys())}")
                    ...
                    if not unsat_info.missing_integrations:
                        raise UnresolvableBundleError(
                            "Cannot find valid bundle topology (no missing integrations identified)",
                            best_bundle=base
                        )
    ...
    def _apply_constraint(
        ...
        if constraint_type == "required":
            ...
            if app_endpoint in problem_space.endpoint_integration_counts:
                ...
                self.logger.warning(f"Endpoint {app_endpoint} not found in problem space - cannot add required constraint")
                ...
                if constraint_str in problem_space.constraint_names:
                    ...
                    if app_name in applications:
                        ...
                        if endpoint_name in app.charm.endpoints:
                            ...
                            unsat_info.missing_integrations.append((
                                app_name,
                                endpoint_name,
                                interface
                            ))
