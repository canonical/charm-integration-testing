...
class MetaChaosClient(ChaosClient):
    ...
    def fill_disk(self, model: JujuModelHandle, unit: str, path: str, size_mb: int) -> None:
        self._dispatch(
            ...
            lambda tool: _CleanupAction((model.uri, unit), lambda _: tool.cleanup(model, unit, path)),
            self._cleanups,
        )
    ...
    @staticmethod
    def _experiment_cleanup(tool: ChaosClient, model: JujuModelHandle, unit: str) -> _CleanupAction:
        return _CleanupAction((model.uri, unit), lambda path: tool.cleanup(model, unit, path))
    ...
    def _cleanup(pending: list[_CleanupAction], scope: tuple[str, str] | None, path: str) -> None:
        ...
        for action in reversed(tuple(pending)):
            if scope is not None and action.scope != scope:
                continue
            try:
                action.run(path)
