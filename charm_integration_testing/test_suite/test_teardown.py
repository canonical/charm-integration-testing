...
def test_teardown(
    ...
    # the relation has to be torn down from the consuming side first; nothing else removes it on
    # its own. See https://github.com/canonical/charm-integration-testing/issues/939.
    ...
    # For CMR integrations, destroying the offering application doesn't clean up the consuming
    # side's SAAS proxy: juju status keeps listing it indefinitely (eventually as "dead"/
    # "terminated") since nothing removes it automatically. That blocks a later redeploy under the
