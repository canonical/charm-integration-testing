...
def test_scale_down_scale_up_charm(juju_client: JujuClient, model: str, requirer_application: str):
    # Get units
    num_units = juju_client.num_units(requirer_application, model=model)

    # Remove units
    juju_client.scale_application(requirer_application, 0, model=model)
    juju_client.idle_for_period(model=model)

    # Rescale application
    juju_client.scale_application(requirer_application, num_units, model=model)
    juju_client.idle_for_period(model=model)
